    ...
    , m_effectBasket(nullptr)
{
    ...
    if (!qEnvironmentVariableIsSet("CAPTHEME_CLASSIC")
        && !KConfigGroup(KSharedConfig::openConfig(), QStringLiteral("CapTheme")).readEntry("WorkspaceInitialized", false)) {
        KdenliveSettings::setTabposition(0);
    }
...
void MainWindow::init()
{
    if (!qEnvironmentVariableIsSet("CAPTHEME_CLASSIC")
        && !KConfigGroup(KSharedConfig::openConfig(), QStringLiteral("CapTheme")).readEntry("WorkspaceInitialized", false)) {
        KdenliveSettings::setWindow_background(QColor(QStringLiteral("#111214")));
    }
